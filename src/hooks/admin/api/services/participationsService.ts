...
export const fetchMissionParticipations = async () => {
  ...
  try {
    // First, fetch all mission participations
    ...
    if (!participations || participations.length === 0) {
      console.log('[fetchMissionParticipations] No participations found');
      return { success: true, participations: [] };
    }
    
    console.log(`[fetchMissionParticipations] Found ${participations.length} participations`);
    ...
    // Create placeholder profiles for any missing users
    userIds.forEach(userId => {
      if (!profilesMap.has(userId)) {
        console.log(`[fetchMissionParticipations] Creating placeholder for missing profile: ${userId}`);
    ...
    console.log('[fetchMissionParticipations] Successfully transformed data:', transformedData.length);
...
export const fetchMissionParticipationsWithFilters = async (
  ...
  try {
    ...
    // Create placeholders for any missing profiles
    userIds.forEach(userId => {
      if (!profilesMap.has(userId)) {
        profilesMap.set(userId, {
          id: userId,
