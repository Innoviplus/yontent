...
export const updateAvatarUrl = async (userId: string, avatarUrl: string): Promise<void> => {
  try {
    ...
    // First check if profile exists using RPC to bypass RLS
    // This ensures we're working directly with the database without RLS constraints
    const { data, error } = await supabase.rpc('get_profile_by_id', { user_id_input: userId });
    ...
    if (data && data.length > 0) {
      ...
      const { error: insertError } = await supabase.rpc('insert_profile_with_avatar', { 
        user_id_input: userId, 
        avatar_url_input: avatarUrl 
      });
